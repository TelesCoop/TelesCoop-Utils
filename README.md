...
- `ssh-key-updater/` : script Python pour mettre à jour la clé SSH d'un utilisateur sur tous les VPS TelesCoop. Lit les employés depuis `employees.yaml` et la liste des serveurs depuis le fichier [hosts](https://github.com/TelesCoop/ansible-ssh-config/blob/main/hosts) du repo `ansible-ssh-config`. Usage : `python3 ssh-key-updater/update_ssh_keys.py <username>` (ajouter `--dry-run` pour tester sans modifier, `--vps vps00 vps01` pour cibler des serveurs spécifiques).
